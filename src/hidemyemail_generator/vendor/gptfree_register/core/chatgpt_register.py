...
class OpenAIAuthClient:
    ...
    async def init_page_email(self, email: str) -> dict:
        ...
        return {
            ...
            "page_path": urlparse(
                str(getattr(final_resp, "url", "") or current_url or "")
            ).path,
        }
    ...
    async def _register_async(
        ...
        *,
        ...
        try:
            ...
            # 4b. Follow the server-selected step.  An external OpenAI OAuth
            ...
            continue_path = urlparse(about_you_url).path
            is_session_callback = bool(
                page_type == "external_url"
                and continue_path.startswith("/api/auth/callback/")
            )
