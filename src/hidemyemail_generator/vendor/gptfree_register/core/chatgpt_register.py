...
# Password setup uses an explicit Strategy chain.  The primary strategy is the
# Settings -> Account reauth contract; HTTP 409 means that contract was not
# accepted for the current passwordless session, so the presenter switches to
# the email-OTP reset strategy and then asks the worker to prove the password in
# a clean session.
PASSWORD_WRITE_ADDED = "added"
PASSWORD_WRITE_RESET = "reset"
PASSWORD_ADD_RESET_FALLBACK_HTTP_STATUSES = frozenset({409})
PASSWORD_ADD_RETRYABLE_HTTP_STATUSES = frozenset(
    {0, 403, 408, 425, 500, 502, 503, 504}
)
    ...
    async def init_page_email(
        ...
        *,
        ...
        if post_login_add_password:
            # Match the Settings -> Account "Add password" reauth contract.
            # The account hint and max_age=0 are part of the state that makes
            # /api/accounts/password/add valid; a normal signup session alone
            # is rejected with HTTP 409 by the current auth service.
            signin_params.update(
                {
                    "connection": "password",
                    "login_hint": email,
                    "reauth": "password",
        ...
        elif not prefer_password_signup:
...
class ChatGPTRegister:
    ...
    async def _reset_existing_password(
        ...
        *,
        ...
        client_id: str,
        refresh_token: str,
        recovery_reason: str = "password_mismatch",
    ) -> tuple[bool, str]:
        """Reset an existing account to the staged password using email OTP."""
        if recovery_reason == "password_add_conflict":
            self._l(
                "  [策略] password/add 返回状态冲突；"
                "切换邮箱 OTP + password/reset 策略..."
            )
        else:
    ...
    async def _complete_password_add_reauth(
        ...
        *,
        ...
        for add_attempt in range(2):
            ...
            if (
                not add_code
                and 200 <= add_status < 300
                and not add_result.get("error")
            ):
                self.password_confirmed = True
            ...
            add_failure = (
                ...
                f"code={add_code or '-'} http={add_status or '-'}"
                + (f" ({add_message})" if add_message else "")
            )
            if add_status in PASSWORD_ADD_RESET_FALLBACK_HTTP_STATUSES:
                reset_ok, reset_error = await self._reset_existing_password(
                    auth,
                    password=password,
                    client_id=client_id,
                    refresh_token=refresh_token,
                    recovery_reason="password_add_conflict",
                )
                if reset_ok:
                    self.password_confirmed = True
                    self._l(
