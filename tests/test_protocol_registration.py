...
class ProtocolRegistrationWorkerTests(unittest.TestCase):
    ...
    def test_init_page_csrf_403_refreshes_kookeey_proxy_session(self):
        instances = []
        events = []
        ...
        proxy_url = "http://customer:secret-US-AbCd1234-5m@proxy.example:8080"
        module = SimpleNamespace(ChatGPTRegister=FakeRegister)
        with (
            patch(
                "hidemyemail_generator.protocol_registration_worker._load_core",
        ...
        self.assertEqual(len(instances), 2)
        self.assertEqual(instances[0].proxy, proxy_url)
        self.assertNotEqual(instances[1].proxy, proxy_url)
        self.assertIn(
            "secret-US-",
            unquote(urlsplit(instances[1].proxy).password or ""),
        )
        self.assertEqual(
            [instance.impersonate for instance in instances],
            ["firefox144", "chrome136"],
        )
        self.assertTrue(
            result["registration_diagnostics"]["transient_init_recovered"]
        )
        self.assertTrue(
            any(
                "CSRF 被边缘防护拒绝" in message
                for _stage, message, _status in events
            )
        )
